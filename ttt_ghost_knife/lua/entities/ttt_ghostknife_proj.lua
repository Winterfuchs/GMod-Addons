...
if CLIENT then
   ENT.PrintName = "thrown Ghost Knife"
  ...
  if SERVER then
      ...
      dmg:SetDamageType(DMG_SLASH)
					...
					if other:IsPlayer() and not other:IsSpec() then
						owner:SetPos(otherPos)
						other:SetPos(ownerPos)
						owner:Give("weapon_ttt_ghostknife")
						self.DidHit = true
						self:Remove()
					end
...
if SERVER then
   function ENT:BecomeWeapon()
	  ...
	  if self.DidHit == false then
      local wep = ents.Create("weapon_ttt_ghostknife")
	  ...
	  wep:Spawn()
	  wep:SetMaterial("Models/effects/splodearc_sheet")
